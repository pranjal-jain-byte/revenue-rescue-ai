...
export async function evaluatePolicy(
  ...
  const result = applyPolicyRules(action, context, rules);
  
  if (result.decision === 'BLOCKED') {
    logger.warn(`Policy BLOCKED: ${result.ruleTriggered}`, { 
      caseId: context.caseId,
      metadata: { action, reason: result.reason }
    });
  }
...
export function evaluatePolicySync(
  ...
  return applyPolicyRules(action, context, rules);
}

/**
 * Core deterministic rule engine shared by both execution and simulation.
 */
